...
static void max7219_init(void)
{
    ...
    gpio_set_level(PIN_CS, 0);
    for (int i = 0; i < NDISPLAY; i++)
        max7219_send(0x0B, 0x07); // 8 filas
    gpio_set_level(PIN_CS, 1);

    gpio_set_level(PIN_CS, 0);
    for (int i = 0; i < NDISPLAY; i++)
        max7219_send(0x0A, 0x0F); // brillo
    gpio_set_level(PIN_CS, 1);

    gpio_set_level(PIN_CS, 0);
    for (int i = 0; i < NDISPLAY; i++)
        max7219_send(0x0C, 0x01); // on
    gpio_set_level(PIN_CS, 1);
}
//----------------------------------------------------------------------
    ...
    while (s[k])
    {
        ...
        else
        {
            ...
            // símbolos
                ...
                glyph = symbols[SYM_AT];
            else if (ch == '<')
                glyph = symbols[SYM_MINOR];
            else if (ch == '>')
                glyph = symbols[SYM_MORE];
            else if ((uint8_t)ch == 0xE2 && (uint8_t)s[k] == 0x82 && (uint8_t)s[k + 1] == 0xAC)
            {
                ...
                glyph = symbols[SYM_PERCENTAGE];
            else if (ch == '(')
                glyph = symbols[SYM_OPEN_PARENTESIS];
            else if (ch == ')')
                glyph = symbols[SYM_CLOSE_PARENTESIS];
            else if (ch == '#')
                glyph = symbols[SYM_HASTAG];
            else
...
void app_main(void)
{
    ...
    char cadena[] = "Bienvenidos al laboratorio de la asignatura de SISTEMAS EMPOTRADOS Curso 26/27....";
