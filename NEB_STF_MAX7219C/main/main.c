...
static const uint8_t digits[10][8] = {
    ...
    {0x3E,0x63,0x03,0x1E,0x30,0x60,0x7F,0x00},
...
static const uint8_t symbols[][8] = {
    ...
    {0x00,0x02,0x04,0x08,0x10,0x20,0x40,0x00}, // /  barra
    {0x00,0x00,0x00,0x00,0x00,0x00,0x7E,0x00}, // _  subrayado
...
static void max7219_init(void)
{
    ...
    gpio_set_level(PIN_CS, 0);
    for (int i = 0; i < NDISPLAY; i++) max7219_send(0x0A, 0x08); // brillo
    gpio_set_level(PIN_CS, 1);
...
void init_M(const char *s, uint8_t *M)
{
    ...
    while (s[k])
    {
        ...
        if ((uint8_t)s[k] == 0xC3 && (uint8_t)s[k+1] == 0xB1) {
            glyph = letter_enye_minor;
            k += 2;
        }
        // UTF-8 Ñ
        else if ((uint8_t)s[k] == 0xC3 && (uint8_t)s[k+1] == 0x91) {
            glyph = letter_enye_upper;
            k += 2;
        }
        else {
            char ch = s[k++];
            ...
            if (ch >= '0' && ch <= '9')         
                ...
                glyph = symbols[SYM_DOT];                      //símbolos
            else if (ch == ',')
                glyph = symbols[SYM_COMMA];
            else if (ch == ':')
                glyph = symbols[SYM_COLON];
            else if (ch == ';')
                glyph = symbols[SYM_SEMICOLON];
            else if (ch == '-')
                glyph = symbols[SYM_MINUS];
            else if (ch == '/')
                glyph = symbols[SYM_SLASH];
            else if (ch == '_')
                glyph = symbols[SYM_UNDERSCORE];
            else if (ch == '!')
                glyph = symbols[SYM_EXCL];
            else if (ch == '?')
                glyph = symbols[SYM_QUESTION];
            else
...
void display_N_digits(uint8_t *M)
{
    ...
    for (int row = 0; row < 8; row++) 
    {
        ...
        {
            max7219_send(row + 1, M[row+8*display]);
        }
        gpio_set_level(PIN_CS, 1);
    }
}
...
void app_main(void)
{
    ...
    char cadena[] = "1234567890 abcdefghijklmnñopqrstuvwxyz ABCDEFGHIJKLMNÑOPQRSTUVWXYZ .,:;-/_!?";
    
    //reserva memoria
    uint8_t *M = (uint8_t *)malloc(strlen(cadena)*8);

    //calcula la longitud de la cadena
    int lcadena =  strlen(cadena)-2;

    //rellena M con las líneas correspondientes de cadena
    init_M(cadena,M);
...
// //--------------------------------------------------------------------
// void init_M2(const char *s, uint8_t *M)
// {
...
//             glyph = digits[ch - '0'];
//         else if (ch >= 'a' && ch <= 'z')
//             glyph = letters_minor[ch - 'a'];
//         else if (ch >= 'A' && ch <= 'Z')
//             glyph = letters_upper[ch - 'A'];
//         else if (ch == ' ')
//             glyph = space;
//         else if (ch == '.')
//             glyph = symbols[SYM_DOT];
//         else if (ch == ',')
//             glyph = symbols[SYM_COMMA];
//         else if (ch == ':')
//             glyph = symbols[SYM_COLON];
...
//     /*
//     for (int row = 0; row < 8; row++) 
//         M[row] = digits[s[0] - '0'][row];                

//     for (int row = 0; row < 8; row++) 
//         M[row+8] = digits[s[1] - '0'][row];        

//     for (int row = 0; row < 8; row++) 
//         M[row+16] = digits[s[2] - '0'][row];                

//     for (int row = 0; row < 8; row++) 
//         M[row+24] = digits[s[3] - '0'][row]; 
//     */
// }