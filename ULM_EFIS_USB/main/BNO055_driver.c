 ...
 *   - Configuración de un remapeo base fijo.
 *   - Composición LSB/MSB.
...
/*
 * El bus I2C se inicializa una sola vez en main.c y se comparte con MPU6050.
 */
...
#define BNO055_GYRO_LSB_PER_DPS 32.8f
...
uint8_t s_operation_mode_reg = BNO055_MODE_AMG;
...
esp_err_t bno055_apply_vertical_mode(void)
{
    uint8_t map_config = BNO055_AXIS_CONFIG_VERTICAL;
    uint8_t map_sign = BNO055_AXIS_SIGN_VERTICAL;
    ...
    ESP_LOGI(TAG, "Orientacion base del BNO055: V (MAP=0x%02X SIGN=0x%02X)",
             map_config,
             map_sign);
...
static esp_err_t bno055_read_vector_scaled(uint8_t first_register, float scale, bno055_vector3f_t *vector)
{
...
esp_err_t bno055_driver_init(void)
{
    ...
    /* Arranque seguro en la orientación vertical validada en el EFIS. */
    ESP_RETURN_ON_ERROR(bno055_apply_vertical_mode(), TAG, "No se pudo aplicar la orientacion V");
    ...
    bno055_operation_mode_t mode)
{
    ...
    else if (mode == BNO055_OPERATION_MODE_IMUPLUS)
    {
        target_mode = BNO055_MODE_IMU;
    }
    else if (mode == BNO055_OPERATION_MODE_NDOF_FMC_OFF)
    {
        target_mode = BNO055_MODE_NDOF_FMC_OFF;
    }
    else
    {
        return ESP_ERR_INVALID_ARG;
...
{
    if (s_operation_mode_reg == BNO055_MODE_AMG)
        return BNO055_OPERATION_MODE_AMG;

    if (s_operation_mode_reg == BNO055_MODE_IMU)
        return BNO055_OPERATION_MODE_IMUPLUS;

    if (s_operation_mode_reg == BNO055_MODE_NDOF_FMC_OFF)
        return BNO055_OPERATION_MODE_NDOF_FMC_OFF;

    return BNO055_OPERATION_MODE_NDOF;
}
...
{
    return bno055_read_vector_scaled(BNO055_REG_ACCEL_DATA_X_LSB,
...
{
    return bno055_read_vector_scaled(BNO055_REG_GYRO_DATA_X_LSB,
                                     BNO055_GYRO_LSB_PER_DPS,
                                     gyro_dps);
}

/* -------------------------------------------------------------------------- */
esp_err_t bno055_driver_read_linear_acceleration(bno055_vector3f_t *linear_acceleration_ms2)
{
...
/* -------------------------------------------------------------------------- */
esp_err_t bno055_driver_read_gravity(bno055_vector3f_t *gravity_ms2)
{
    return bno055_read_vector_scaled(
