...
La validez de comunicaciones y la validez del FIX se tratan por separado.

### Ground Speed

Existe un instrumento analógico dedicado **GS**, en knots, con leyenda `KNOTS`. Funciona a partir de la velocidad GPS.

### Girodireccional / Ground Track

El girodireccional utiliza ahora el **Ground Track del GPS**, no el heading de la IMU.

Una muestra nueva se acepta únicamente cuando el GPS tiene información válida y la velocidad es suficientemente alta para que el course-over-ground sea significativo. Si se pierde temporalmente el heading GPS:

1. se conserva el último Ground Track válido;
2. si nunca se recibió uno válido, se muestra 0°.

Técnicamente el GPS proporciona track/course over ground, no el heading aerodinámico del vehículo.
...
Por compatibilidad, la variable interna puede conservar el nombre histórico `heading_offset_deg`, aunque actualmente representa un valor manual absoluto.
