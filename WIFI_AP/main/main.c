...
static void altitude_test_task(void *pvParameters)
{
    ...
    /* 500 ft/min = 2.54 m/s. */
    const float rate_mps =
        ALTITUDE_TEST_RATE_FPM *
        FEET_TO_METERS /
        SECONDS_PER_MINUTE;
...
void app_main(void)
{
     ...
     * DataLogger SPIFFS.
     *
     * No se aborta el EFIS si el registrador no puede montarse: el resto
     * de instrumentos y la interfaz web continúan funcionando.
     */
