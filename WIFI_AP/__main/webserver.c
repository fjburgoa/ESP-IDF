...
static esp_err_t download_log_get_handler(httpd_req_t *req)
{
    ...
    if (status.recording)
    {
        ...
        return httpd_resp_sendstr(req, "Detenga la grabacion antes de descargar el fichero.");
    }
    ...
    if (!status.file_available)
    {
        ...
        httpd_resp_set_type(req, "text/plain; charset=utf-8");
        return httpd_resp_sendstr(req, "No existe ningun registro de aceleracion.");
    }

    FILE *file = fopen(DataLogger_get_file_path(), "r");
    if (file == NULL)
    {
        httpd_resp_set_status(req, "500 Internal Server Error");
        return httpd_resp_sendstr(req, "No se pudo abrir el registro.");
    }

    httpd_resp_set_type(req, "text/csv; charset=utf-8");

    httpd_resp_set_hdr(req, "Content-Disposition", "attachment; filename=\"aceleracion.csv\"");
