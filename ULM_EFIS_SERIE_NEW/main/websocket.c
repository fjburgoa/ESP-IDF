...
static uint32_t s_imu_mode = BNO055_DEFAULT_OPERATION_MODE;
...
static void settings_load(void)
{
    ...
    uint32_t imu_mode = BNO055_DEFAULT_OPERATION_MODE;
    ...
    if (imu_mode > (uint32_t)BNO055_OPERATION_MODE_NDOF)
    {
        imu_mode = BNO055_DEFAULT_OPERATION_MODE;
    }

    esp_err_t imu_err =
        BNO055_set_operation_mode((bno055_operation_mode_t)imu_mode);

    if (imu_err != ESP_OK)
    {
        ESP_LOGW(
            TAG,
            "No se pudo recuperar el modo IMU: %s",
            esp_err_to_name(imu_err));

        imu_mode = (uint32_t)BNO055_get_operation_mode();
    }

    /*
     * Se aplica después al BNO055 para que la salida quede expresada
    ...
    ESP_LOGI(TAG,
             ...
             " deg, heading offset=%" PRIu32 " deg, montaje=%s, IMU=%s",
             (double)qnh,
             pitch_offset,
             heading_offset,
             mount_mode == (uint32_t)BNO055_MOUNT_HORIZONTAL ? "H" : "V",
             imu_mode == (uint32_t)BNO055_OPERATION_MODE_NDOF ? "NDOF" : "AMG");
}
//----------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------
//----------------------------------------------------------------------------------
static void imu_mode_toggle(void)
{
    const bno055_operation_mode_t current = BNO055_get_operation_mode();

    const bno055_operation_mode_t new_mode =
        (current == BNO055_OPERATION_MODE_NDOF)
            ? BNO055_OPERATION_MODE_AMG
            : BNO055_OPERATION_MODE_NDOF;

    esp_err_t err = BNO055_set_operation_mode(new_mode);

    if (err != ESP_OK)
    {
        ESP_LOGW(TAG, "No se pudo cambiar el modo IMU: %s", esp_err_to_name(err));
        return;
    }

    portENTER_CRITICAL(&s_settings_mux);
    s_imu_mode = (uint32_t)new_mode;
    portEXIT_CRITICAL(&s_settings_mux);

    (void)settings_save_u32(NVS_KEY_IMU_MODE, (uint32_t)new_mode);

    ESP_LOGI(TAG, "Modo IMU actualizado y guardado: %s",
             new_mode == BNO055_OPERATION_MODE_NDOF ? "NDOF" : "AMG");
}
//----------------------------------------------------------------------------------
...
static esp_err_t websocket_handler(httpd_req_t *req)
{
    ...
    if (err == ESP_OK)
    {
        ...
        else if (strcmp((char *)payload, "IMU_MODE_TOGGLE") == 0)
        {
            imu_mode_toggle();
        }
