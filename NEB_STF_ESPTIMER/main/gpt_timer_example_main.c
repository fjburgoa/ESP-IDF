...
{
    uint32_t freq = 0;
    esp_clk_tree_src_get_freq_hz(SOC_CPU_CLK_SRC_PLL, ESP_CLK_TREE_SRC_FREQ_PRECISION_EXACT, &freq);
    return freq;
}
// ----------------------------------------------------------------------------------------------------
...
{
    uint32_t freq = 0;
    esp_clk_tree_src_get_freq_hz(SOC_MOD_CLK_APB, ESP_CLK_TREE_SRC_FREQ_PRECISION_EXACT,  &freq);
    return freq;
}
// ----------------------------------------------------------------------------------------------------
...
{
    uint32_t freq = 0;
    esp_clk_tree_src_get_freq_hz(SOC_MOD_CLK_XTAL, ESP_CLK_TREE_SRC_FREQ_PRECISION_EXACT, &freq);
    return freq;
}
// ----------------------------------------------------------------------------------------------------
static uint32_t timer_base_hz(bool use_xtal, uint32_t freq_xtal,  uint32_t freq_apb)
{
    return use_xtal ? freq_xtal : freq_apb;
}
// ----------------------------------------------------------------------------------------------------
static uint32_t timer_freq_hz(uint32_t divider, bool use_xtal, uint32_t freq_xtal, uint32_t freq_apb)
{
...
// ------------------------------------------------------
// Función principal para imprimir la configuración
// ------------------------------------------------------
...
bool IRAM_ATTR GPT_TIMER_ISR(gptimer_handle_t timer, const gptimer_alarm_event_data_t *event, void *user_data) 
{
    Flag_timer = 1;
...
void app_main(void) 
{
    ...
    gptimer_config_t timer_config = {
        .clk_src       = GPTIMER_CLK_SRC_XTAL, //GPTIMER_CLK_SRC_DEFAULT,  // Reloj por defecto (APB 80 MHz)
        .direction     = GPTIMER_COUNT_UP,         // Contar hacia arriba
        .resolution_hz = 611,                     // 1 MHz (1us por tick)
    };
