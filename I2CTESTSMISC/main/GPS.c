...
#define GPS_UART_TX_GPIO 47
#define GPS_UART_RX_GPIO 45
...
static bool gps_ubx_set_rate_5hz(void)
{
...
static void gps_configure_receiver(void)
{
    ...
    ESP_LOGI(
        TAG,
        "Comunicación verificada a 115200 baud: %s",
        line);
    ...
    if (gps_ubx_save_configuration())
    {
        ESP_LOGI(
            TAG,
            "Configuración guardada por el receptor");
    }
    else
    {
        ESP_LOGW(
...
esp_err_t GPS_start(void)
{
    ...
    {
        ESP_LOGE(TAG, "No se detectó NMEA a 9600 baud");
        return ESP_ERR_NOT_FOUND;
    }

    ESP_LOGI(TAG, "GPS detectado a 9600 baud: %s", first_line);
