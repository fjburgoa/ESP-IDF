...
static esp_err_t hmc5883l_init(void)
{
    ...
    ret = hmc5883l_write_register(HMC5883L_REG_CONFIG_A, HMC5883L_AVERAGING_8 | HMC5883L_RATE_15HZ | HMC5883L_BIAS_NORMAL);
    if (ret != ESP_OK)
        return ret;
...
#define HMC5883L_LSB_PER_GAUSS 1090.0f
        ...
        if (hmc5883l_data_ready(&ready) == ESP_OK && ready)
        {
            ...
            {
                float x_gauss = raw_data.x / HMC5883L_LSB_PER_GAUSS;
                float y_gauss = raw_data.y / HMC5883L_LSB_PER_GAUSS;
                float z_gauss = raw_data.z / HMC5883L_LSB_PER_GAUSS;
...
#define MAG_Y_OFFSET (-126.5f)
                ...
                printf("Raw X=%d Y=%d Z=%d | Field X=%.3f Y=%.3f Z=%.3f G |B|=%.3f G H=%.2f deg\n",
                       raw_data.x,
                       raw_data.y,
                       raw_data.z,
                       x_gauss,
