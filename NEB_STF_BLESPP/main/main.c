...
#define SPP_SVC_UUID  0xABF0
#define SPP_CHR_UUID  0xABF1

static uint16_t conn_h     = BLE_HS_CONN_HANDLE_NONE;
static uint16_t chr_h      = 0;
static bool     notify_en  = false;
static uint8_t  own_addr_type;
static const char *TAG = "BLE_ULTRA";
...
static int gap_cb(struct ble_gap_event *e, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        if (e->connect.status == 0) {
            ...
            ESP_LOGI(TAG, "Conectado, handle=%d", conn_h);
        } else {
            ESP_LOGI(TAG, "Fallo de conexión, reintentando");
    ...
    case BLE_GAP_EVENT_SUBSCRIBE:
        ...
        ESP_LOGI(TAG, "Notify=%d", notify_en);
        break;
...
static void adv_start(void)
{
    ...
    const char *name = "*ESP32S3*";
    f.name = (uint8_t *)name;
    f.name_len = strlen(name);
    f.name_is_complete = 1;

    f.uuids16 = (ble_uuid16_t[]) { BLE_UUID16_INIT(SPP_SVC_UUID) };
    f.num_uuids16 = 1;
