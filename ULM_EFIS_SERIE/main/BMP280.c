...
esp_err_t bmp280_write8(uint8_t reg, uint8_t data)
{
    ...
    return i2c_master_write_to_device(EFIS_I2C_PORT, BMP280_ADDR, buf, sizeof(buf), pdMS_TO_TICKS(EFIS_I2C_TIMEOUT_MS));
}
/*------------------------------------------------------------------------------*/
esp_err_t bmp280_read(uint8_t reg, uint8_t *data, size_t len)
{
    return i2c_master_write_read_device(EFIS_I2C_PORT, BMP280_ADDR, &reg, 1, data, len, pdMS_TO_TICKS(EFIS_I2C_TIMEOUT_MS));
}
/*------------------------------------------------------------------------------*/
...
esp_err_t bmp280_init(void)
{
    ...
    ESP_ERROR_CHECK(bmp280_write8(REG_RESET, BMP280_RESET_VALUE));
     ...
     *   spi3w  = 0
     */
    ESP_ERROR_CHECK(bmp280_write8(REG_CONFIG, BMP280_CONFIG_VALUE));
     ...
     *   modo normal
     */
    ESP_ERROR_CHECK(bmp280_write8(REG_CTRL_MEAS, BMP280_CTRL_MEAS_VALUE));
...
esp_err_t bmp280_start(void)
{
    ...
    // Crear la tarea
    BaseType_t ok = xTaskCreate(BMP280Task, "alt_meas", BMP280_TASK_STACK_SIZE, &ucParamToPass, BMP280_TASK_PRIORITY, &xHandleBMP);
