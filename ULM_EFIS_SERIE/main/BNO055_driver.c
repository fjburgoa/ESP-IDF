...
static esp_err_t bno055_write_u8(uint8_t reg, uint8_t value)
{
    ...
    return i2c_master_write_to_device(
        ...
        data,
        sizeof(data),
        pdMS_TO_TICKS(EFIS_I2C_TIMEOUT_MS));
}
...
static esp_err_t bno055_detect_once(void)
{
    ...
    for (size_t i = 0U; i < sizeof(addresses) / sizeof(addresses[0]); ++i)
    {
        ...
        /*
         * 0x28/0x29 son las dos direcciones validas del BNO055.
         * Durante power-up puede no responder temporalmente; no se considera
         * fatal hasta agotar BNO055_DETECT_TIMEOUT_MS.
         */
        ESP_LOGW(
            TAG,
            "Sin BNO055 valido en 0x%02X: err=%s, CHIP_ID=0x%02X",
            s_bno055_address,
            esp_err_to_name(err),
            chip_id);
    }
